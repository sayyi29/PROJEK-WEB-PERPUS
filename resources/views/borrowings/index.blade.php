    ...
    <div class="space-y-10">
        ...
        <div class="premium-card rounded-[2.5rem] overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#FDFCF9] border-b border-[#E8E4D9]">
                            <th class="px-8 py-6 text-[10px] font-black text-[#062C2C] uppercase tracking-widest">Borrower Protocol</th>
                            <th class="px-8 py-6 text-[10px] font-black text-[#062C2C] uppercase tracking-widest">Asset Details</th>
                            <th class="px-8 py-6 text-[10px] font-black text-[#062C2C] uppercase tracking-widest">Timeline</th>
                            ...
                            <tr class="hover:bg-[#FDFCF9] transition-colors group">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p class="text-sm font-black text-[#062C2C] uppercase">{{ $borrowing->user->name }}</p>
                                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">UID: {{ str_pad($borrowing->user->id, 4, '0', STR_PAD_LEFT) }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <p class="text-sm font-bold text-[#062C2C]">{{ $borrowing->book->title }}</p>
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex flex-col gap-1">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <p class="text-[10px] font-bold text-[#062C2C]">{{ $borrowing->borrow_date->format('d M Y') }}</p>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span class="w-1.5 h-1.5 rounded-full bg-rose-400"></span>
                                            <p class="text-[10px] font-bold text-rose-900 uppercase">Due: {{ $borrowing->due_date->format('d M Y') }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <div class="flex justify-center">
                                        @php
                                            $statusStyle = match($borrowing->status) {
                                                'borrowed' => 'bg-emerald-50 text-emerald-700 border-emerald-100',
                                                'overdue' => 'bg-rose-50 text-rose-900 border-rose-100',
            ...
            @if($borrowings->hasPages())
                <div class="px-8 py-6 bg-[#FDFCF9] border-t border-[#E8E4D9]">
                    {{ $borrowings->links() }}
                </div>
            @endif
        </div>
