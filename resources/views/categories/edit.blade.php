<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h2 class="font-black text-4xl text-white tracking-tighter uppercase leading-none mb-1">Edit Kategori</h2>
                <p class="text-indigo-400 font-bold text-[10px] tracking-[0.4em] uppercase opacity-70">Pembaruan Klasifikasi Koleksi</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-6">
            <div class="space-y-10">
                <div class="px-6">
                    <h3 class="text-2xl font-black text-white tracking-tight uppercase tracking-widest">Detail Klasifikasi</h3>
                    <p class="text-indigo-400 font-bold text-xs uppercase tracking-[0.2em] mt-2">ID Kategori: #{{ $category->id }}</p>
                </div>

                <form action="{{ route('categories.update', $category->id) }}" method="POST" class="space-y-10">
                    @csrf
                    @method('PUT')
                    <div class="glass-card p-12 rounded-[3.5rem] border border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.4)] relative overflow-hidden group">
                        ...
                        <div class="relative space-y-8">
                            ...
                            <div class="space-y-3">
                                ...
                                <input type="text" name="rack" value="{{ old('rack', $category->rack) }}" required
                                    class="w-full bg-white/5 border border-white/10 rounded-3xl px-8 py-5 text-white focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-300 font-medium text-lg placeholder:text-white/10"
